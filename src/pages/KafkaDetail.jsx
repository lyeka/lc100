/**
 * [INPUT]: react-router (useParams, Link), @/data/kafkaInterviews, @/components/ui/button, @/components/CodeBlock, @/components/LearnedStamp, @/components/OpenInChatGPT, @/lib/interviewPrompts, lucide-react (GraduationCap)
 * [OUTPUT]: KafkaDetail 古典书页阅读页
 * [POS]: 路由 /kafka/:id，Kafka 面试题阅读体验核心
 *        书页容器 bg-card shadow-2xl 浮于深色桌面
 *        阅读节奏：装饰线章节标题 → 背景说明(可选) → 参考要点(中文数字) → 代码示例(可选) → 追问(可选) → 纯文字导航
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */
import { useParams, Link } from 'react-router'
import { kafkaInterviews, kafkaInterviewMap } from '@/data/kafkaInterviews'
...
export function KafkaDetail() {
  const { id } = useParams()
  const question = kafkaInterviewMap.get(id)
  ...
  if (!question) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-20 text-center">
        ...
        <Link to="/kafka">
          <Button variant="outline" className="mt-4">返回 Kafka 目录</Button>
        </Link>
      </main>
    )
  }
  ...
  /* ── 前后题导航（基于数组索引）── */
  const idx = kafkaInterviews.findIndex(q => q.id === id)
  const prev = idx > 0 ? kafkaInterviews[idx - 1] : null
  const next = idx < kafkaInterviews.length - 1 ? kafkaInterviews[idx + 1] : null
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link to="/kafka" className="text-sm text-muted-foreground hover:text-primary transition-colors">
          ← Kafka 目录
        </Link>
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          {prev ? (
            <Link to={`/kafka/${prev.id}`} className="hover:text-primary transition-colors">
              {prev.id}
            </Link>
          ) : (
          ...
          {next ? (
            <Link to={`/kafka/${next.id}`} className="hover:text-primary transition-colors">
              {next.id}
            </Link>
          ) : (
            <span className="opacity-30">···</span>
      ...
      <div className="bg-card shadow-2xl px-8 sm:px-16 py-12">
        ...
        <div className="flex items-center justify-center gap-2 py-4 flex-wrap">
          <LearnedStamp section="kafka" id={id} total={kafkaInterviews.length} />
          <OpenInChatGPT prompt={buildExplainPrompt(question)} />
          <OpenInChatGPT
            prompt={buildAnswerPrompt(question, ROLE_MAP.kafka)}
            label="ChatGPT 解答"
            icon={GraduationCap}
          ...
          <>
            ...
            <section>
              ...
              <div className="px-4 sm:px-8">
                <CodeBlock code={question.codeExample.code} language={question.codeExample.language || 'bash'} />
