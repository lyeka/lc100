/**
 * [INPUT]: react-router (useParams, Link), @/data/mysqlInterviews, @/components/ui/button, @/components/CodeBlock, @/components/LearnedStamp, @/components/OpenInChatGPT
...
function buildPrompt(q) {
  ...
  if (q.codeExample) {
    parts.push('', '【代码示例】', '```' + (q.codeExample.language || 'sql'), q.codeExample.code, '```')
...
export function MysqlDetail() {
  const { id } = useParams()
  const question = mysqlInterviewMap.get(id)
  ...
  if (!question) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-20 text-center">
        ...
        <Link to="/mysql">
          <Button variant="outline" className="mt-4">返回 MySQL 目录</Button>
        </Link>
      </main>
    )
  }
  ...
  /* ── 前后题导航（基于数组索引）── */
  const idx = mysqlInterviews.findIndex(q => q.id === id)
  const prev = idx > 0 ? mysqlInterviews[idx - 1] : null
  const next = idx < mysqlInterviews.length - 1 ? mysqlInterviews[idx + 1] : null
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link to="/mysql" className="text-sm text-muted-foreground hover:text-primary transition-colors">
          ← MySQL 目录
        </Link>
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          {prev ? (
            <Link to={`/mysql/${prev.id}`} className="hover:text-primary transition-colors">
              {prev.id}
            </Link>
          ) : (
          ...
          {next ? (
            <Link to={`/mysql/${next.id}`} className="hover:text-primary transition-colors">
              {next.id}
            </Link>
          ) : (
            <span className="opacity-30">···</span>
          ...
          <>
            ...
            <section>
              ...
              <div className="px-4 sm:px-8">
                <CodeBlock code={question.codeExample.code} language={question.codeExample.language || 'sql'} />
