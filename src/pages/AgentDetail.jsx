/**
 * [INPUT]: react-router (useParams, Link), @/data/interviews, @/components/ui/button, @/components/LearnedStamp, @/components/OpenInChatGPT, @/lib/interviewPrompts, lucide-react (GraduationCap)
...
export function AgentDetail() {
  const { id } = useParams()
  const question = interviewMap.get(id)
  ...
  if (!question) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-20 text-center">
        ...
        <Link to="/agent">
          <Button variant="outline" className="mt-4">返回 Agent 目录</Button>
        </Link>
      </main>
    )
  }
  ...
  /* ── 前后题导航（基于数组索引）── */
  const idx = interviews.findIndex(q => q.id === id)
    ...
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          {prev ? (
            <Link to={`/agent/${prev.id}`} className="hover:text-primary transition-colors">
              {prev.id}
            </Link>
          ) : (
          ...
          {next ? (
            <Link to={`/agent/${next.id}`} className="hover:text-primary transition-colors">
              {next.id}
            </Link>
          ) : (
            <span className="opacity-30">···</span>
      ...
      <div className="bg-card shadow-2xl px-8 sm:px-16 py-12">
        ...
        <div className="flex items-center justify-center gap-2 py-4 flex-wrap">
          ...
          <OpenInChatGPT
            prompt={buildAnswerPrompt(question, ROLE_MAP.agent)}
            label="ChatGPT 解答"
            icon={GraduationCap}
