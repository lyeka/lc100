/**
 * [INPUT]: react-router (useSearchParams, Link), react (useState), @/data/redisInterviews, @/lib/progress (useProgress)
 * [OUTPUT]: RedisHome — Redis 面试题古典目录页
 * [POS]: 路由 /redis，按 8 大分类分组的 Table of Contents，纯文字筛选
...
export function RedisHome() {
  const [params, setParams] = useSearchParams()
  const activeCategory = params.get('category') || ''
  const activeLevel    = params.get('level') || ''
  const showUnlearned  = params.get('unlearned') === '1'
  const [catOpen, setCatOpen] = useState(false)
  const { isLearned } = useProgress('redis', redisInterviews.length)

  /* ── 筛选 ── */
  const filtered = redisInterviews.filter(q => {
  ...
  const grouped = redisInterviewCategories.reduce((acc, cat) => {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div>
          ...
          {catOpen && (
            <div className="mt-3 flex flex-wrap items-center justify-center gap-x-1 gap-y-1.5 text-xs leading-relaxed">
              ...
              {redisInterviewCategories.map(cat => (
      ...
      {/* ════════════════════════════════════════
         结果计数
         ════════════════════════════════════════ */}
      {filtered.length !== redisInterviews.length && (
