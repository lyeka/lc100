...
React 19 + Vite 7 + TailwindCSS v4 + shadcn/ui (Amethyst Haze) + react-router + framer-motion + prism-react-renderer
...
<directory>
src/              - 应用源码
  components/     - 全局组件 (Header, Footer, ProblemCard, CodeBlock + ui/ shadcn)
...
## 数据 Schema
每个 JSON: `{ id, leetcodeId, title, category, difficulty, description, hint: { core, keyPoints[] }, solutions: [{ name, code, timeComplexity, spaceComplexity }] }`
